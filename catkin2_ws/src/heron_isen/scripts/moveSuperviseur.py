...
def done_cb(status, result):
    ...
    if status == 4:
        ...
        if ((desiredPose[0] - getCurrentPose()[0] < 0.60) and (desiredPose[1] - getCurrentPose()[1] < 0.60)):
            publish(ipsuperviseur, port, "Service/Arrived/" + derniertopic, my_ip + "/" + position, 2)
        else:
...
#coucou=0
#def battery():
	#coucou=os.system('rosrun heron_isen battery.py')
...
def pingRobot():
	...
	b.listener()
        #os.system('rosrun heron_isen battery.py')

	print ("Envoi du ping :", my_ip+"/"+str(b.battery))
