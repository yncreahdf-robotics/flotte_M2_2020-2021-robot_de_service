...
#define DIST_ROTATION 0.35
#define DIST_BREAK 0.05
...
RobotinoSafety::RobotinoSafety():
	...
	node_loop_rate_ (40)
{
	//pub = nh_.advertise<geometry_msgs::Twist>("/cmd_vel_obstacle", 1000);
	...
	//robotino_cmd_vel_sub_ = nh_.subscribe("/cmd_vel", 100, &RobotinoSafety::robotinoCmdVelCallback, this);
	...
	//distance_sensors = nh_.subscribe("/distance_sensors", 100, &RobotinoSafety::distanceSensorsCallback, this);	
	...
	nh_.param( "node_loop_rate", node_loop_rate_, 40 );
...
/*void RobotinoSafety::distanceSensorsCallback(const sensor_msgs::PointCloudConstPtr& msg)
{
	...
	float cmd_rotation = 0;
        ...
        if(distance[0] < DIST_ROTATION)
        {
                cmd_rotation = distance[0] / DIST_ROTATION;
